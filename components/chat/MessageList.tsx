                        ...
                        <div className={`max-w-xl lg:max-w-3xl px-5 py-4 rounded-2xl relative shadow-sm border ${msg.isError ? 'bg-red-50 text-red-800 border-red-200' : msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-sm border-blue-600' : 'bg-white text-gray-800 rounded-bl-sm border-gray-200'}`}>
                            ...
                            {!redirectData && (
                                <div className="absolute top-2 left-2 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity no-print z-10">
                                    <button onClick={() => onPinMessage(msg)} className="p-1.5 bg-gray-200/50 hover:bg-gray-300 rounded-full text-gray-500 hover:text-gray-700 disabled:opacity-50 transition-colors" title={isPinned ? "تم التثبيت" : "تثبيت الرسالة"} disabled={isPinned}>
                                    ...
                                    {msg.role === 'model' && (
                                        <>
                                            <button onClick={() => handleSpeak(msg.content, msg.id)} className={`p-1.5 bg-gray-200/50 rounded-full hover:bg-gray-300 transition-colors ${speakingMessageId === msg.id ? 'text-green-600 animate-pulse' : 'text-gray-500'}`}>
                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" /></svg>
                                            </button>
                                            <button onClick={() => handleCopy(msg.content, msg.id)} className="p-1.5 bg-gray-200/50 rounded-full text-gray-500 hover:bg-gray-300 transition-colors">
                            ...
                            {redirectData ? (
                                <div className="bg-red-50 border-2 border-red-200 p-4 rounded-xl text-center">
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <p className="text-red-700 text-sm">{redirectData.reason}</p>
                                        {onConvertCaseType && (
                                            <button onClick={() => onConvertCaseType(redirectData.redirect)} className="mt-2 px-6 py-2.5 bg-red-600 text-white font-bold rounded-lg shadow-lg">نقل القضية فوراً</button>
                                        )}
                                    </div>
                                </div>
                            ) : (
                                ...
                                <div className="mt-4 pt-3 border-t border-gray-200 no-print">
                                    <div className="flex flex-wrap gap-2">
                                        {nextActions.map((action, idx) => (
                                            <button key={idx} onClick={() => onFollowUpAction(action.mode, action.prompt)} className="px-4 py-2 bg-gray-50 hover:bg-blue-50 border border-gray-300 rounded-lg text-sm text-gray-700 shadow-sm transition-all">
                                                {action.label}
                                            </button>
                                        ))}
                                    </div>
                            ...
                            {hasGrounding && !redirectData && (
                                <div className="mt-6 pt-4 border-t border-gray-200 bg-gray-50 rounded-lg p-3 -mx-2 no-print">
                                    <p className="text-xs font-bold text-blue-700 mb-3 flex items-center">
                                            ...
                                            chunk.web && (
                                                <a key={idx} href={chunk.web.uri} target="_blank" rel="noopener noreferrer" className="flex items-center p-2 rounded bg-white hover:bg-blue-50 border border-gray-200 transition-all group/link">
                                                    ...
                                                    <div className="flex-grow min-w-0">
                                                        <p className="text-xs font-medium text-gray-800 truncate group-hover/link:text-blue-700">{chunk.web.title || "مصدر قانوني"}</p>
                                                        <p className="text-[10px] text-gray-500 truncate font-mono">{new URL(chunk.web.uri).hostname}</p>
                                                    </div>
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-gray-400 ms-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                                                </a>
                                            )
                                        ))}
                                    </div>
