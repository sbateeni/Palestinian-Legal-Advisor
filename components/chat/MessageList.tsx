    ...
    if (messages.length === 0 && !isLoading) {
        return (
            <div className="text-center text-gray-500 flex flex-col items-center justify-center h-full p-8 no-print">
                ...
                <h2 className="text-3xl font-bold mb-4 text-gray-800 tracking-wide">المستشار القانوني الفلسطيني</h2>
        ...
        <div className="space-y-6 pb-4">
                        ...
                        <div className={`max-w-xl lg:max-w-3xl px-5 py-4 rounded-2xl relative shadow-sm border ${msg.isError ? 'bg-red-50 text-red-800 border-red-200' : msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-sm border-blue-600' : 'bg-white text-gray-800 rounded-bl-sm border-gray-200'}`}>
                            ...
                            {!redirectData && (
                                <div className="absolute top-2 left-2 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity no-print">
                                    ...
                                    {msg.role === 'model' && (
                                        <>
                                            <button onClick={() => handleSpeak(msg.content, msg.id)} className={`p-1.5 bg-gray-200/50 rounded-full hover:bg-gray-300 transition-colors ${speakingMessageId === msg.id ? 'text-green-600 animate-pulse' : 'text-gray-500'}`} aria-label="قراءة صوتية">
                                            ...
                                            <button onClick={() => handleCopy(msg.content, msg.id)} className="p-1.5 bg-gray-200/50 rounded-full text-gray-500 hover:bg-gray-300 transition-colors" aria-label="نسخ">
                                                {copiedMessageId === msg.id ? (
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 text-green-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>
                                                ) : (
                                            ...
                                            <button onClick={() => handleDownload(msg.content, msg.id)} className="p-1.5 bg-gray-200/50 rounded-full text-gray-500 hover:bg-gray-300 transition-colors" aria-label="تحميل">
                            ...
                            {msg.images && msg.images.length > 0 && (
                                <div className="flex flex-wrap gap-2 mb-3">
                                    {msg.images.map((image, index) => (
                                        <img key={index} src={image.dataUrl} alt={`مرفق ${index + 1}`} className="rounded-lg max-w-xs max-h-64 object-contain bg-gray-50 border border-gray-200" />
                                    ))}
                                </div>
                            )}

                            {redirectData ? (
                                <div className="bg-red-50 border-2 border-red-200 p-4 rounded-xl text-center no-print">
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <h3 className="text-xl font-bold text-red-800">تنبيه اختصاص قضائي</h3>
                                        <p className="text-red-700 text-sm leading-relaxed max-w-md">
                                            {redirectData.reason}
                                            <br />
                            ...
                            {nextActions.length > 0 && onFollowUpAction && (
                                <div className="mt-4 pt-3 border-t border-gray-200 no-print">
                                    <p className="text-xs text-gray-500 mb-2 font-medium flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5 me-1 text-blue-500" viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v2H7a1 1 0 100 2h2v2a1 1 0 102 0v-2h2a1 1 0 100-2h-2V7z" clipRule="evenodd" /></svg>
                                        خطوات مقترحة:
                                    </p>
                                    <div className="flex flex-wrap gap-2">
                                        {nextActions.map((action, idx) => (
                                            <button
                                                ...
                                                className="px-4 py-2 bg-gray-50 hover:bg-blue-50 border border-gray-300 hover:border-blue-300 rounded-lg text-sm text-gray-700 hover:text-blue-700 transition-all flex items-center gap-2 shadow-sm hover:shadow-md"
                                            >
                            ...
                            {hasGrounding && !redirectData && (
                                <div className="mt-6 pt-4 border-t border-gray-200 bg-gray-50 rounded-lg p-3 -mx-2 no-print">
                                    <div className="flex items-center justify-between mb-3">
                                        <p className="text-xs font-bold text-blue-700 flex items-center uppercase tracking-wider">
                                        ...
                                        {msg.groundingMetadata?.groundingChunks.map((chunk, idx) => (
                                            chunk.web && (
                                                <a
                                                    ...
                                                    className="flex items-center p-2 rounded bg-white hover:bg-blue-50 border border-gray-200 hover:border-blue-300 transition-all group/link"
                                                >
                                                    <span className="flex-shrink-0 w-5 h-5 rounded bg-blue-100 text-blue-600 flex items-center justify-center text-[10px] me-2 font-mono border border-blue-200">
                                                        {idx + 1}
                                                    </span>
                                                    <div className="flex-grow min-w-0">
                                                        <p className="text-xs font-medium text-gray-800 truncate group-hover/link:text-blue-700">
                                                    ...
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-gray-400 ms-2 group-hover/link:text-blue-500" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                                                </a>
                                            )
                                        ))}
                                    </div>
            ...
            {isLoading && messages.length > 0 && messages[messages.length - 1].role !== 'model' && (
                <div className="flex justify-start no-print">
                    <div className="max-w-xl px-6 py-4 rounded-2xl bg-white border border-gray-200 shadow-sm rounded-bl-none">
