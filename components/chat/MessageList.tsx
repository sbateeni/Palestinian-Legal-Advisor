...
const MessageList: React.FC<MessageListProps> = ({ 
    ...
    if (!Array.isArray(messages) || (messages.length === 0 && !isLoading)) {
        return (
            <div className="text-center text-gray-500 dark:text-slate-300 flex flex-col items-center justify-center h-full p-8 no-print">
                ...
                <h2 className="text-3xl font-bold mb-4 text-gray-800 dark:text-white tracking-wide">المستشار القانوني الفلسطيني</h2>
                <p className="mb-8 max-w-lg text-lg leading-relaxed text-gray-600 dark:text-slate-300">
                        ...
                        <div className={`max-w-xl lg:max-w-3xl px-5 py-4 rounded-2xl relative shadow-sm border transition-all duration-300 ${msg.isError ? 'bg-red-50 dark:bg-red-900/30 text-red-800 dark:text-red-100 border-red-200 dark:border-red-800' : msg.role === 'user' ? 'bg-blue-600 dark:bg-blue-700 text-white rounded-br-sm border-blue-600 dark:border-blue-800' : 'bg-white dark:bg-slate-900 text-gray-800 dark:text-slate-50 rounded-bl-sm border-gray-200 dark:border-slate-700 shadow-md dark:shadow-slate-950'}`}>
                            ...
                            {!redirectData && (
                                <div className="absolute top-2 left-2 flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity no-print z-10">
                                    ...
                                    {msg.role === 'model' && (
                                        <>
                                            <button onClick={() => handleSpeak(msg.content, msg.id)} className={`p-1.5 bg-gray-200/50 dark:bg-slate-800/80 rounded-full hover:bg-gray-300 dark:hover:bg-slate-700 transition-colors ${speakingMessageId === msg.id ? 'text-green-600 animate-pulse' : 'text-gray-500 dark:text-slate-300'}`}>
                                                <svg xmlns="http://www.w3.org/2000/svg" className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15.536 8.464a5 5 0 010 7.072m2.828-9.9a9 9 0 010 12.728M5.586 15H4a1 1 0 01-1-1v-4a1 1 0 011-1h1.586l4.707-4.707C10.923 3.663 12 4.109 12 5v14c0 .891-1.077 1.337-1.707.707L5.586 15z" /></svg>
                                            </button>
                                            <button onClick={() => handleCopy(msg.content, msg.id)} className="p-1.5 bg-gray-200/50 dark:bg-slate-800/80 rounded-full text-gray-500 dark:text-slate-300 hover:bg-gray-300 dark:hover:bg-slate-700 transition-colors">
                            ...
                            {redirectData ? (
                                <div className="bg-red-50 dark:bg-red-900/20 border-2 border-red-200 dark:border-red-800 p-4 rounded-xl text-center">
                                    <div className="flex flex-col items-center gap-3">
                                        ...
                                        <h3 className="text-xl font-bold text-red-800 dark:text-white">تنبيه اختصاص قضائي</h3>
                                        <p className="text-red-700 dark:text-red-100 text-sm">{redirectData.reason}</p>
                                        {onConvertCaseType && (
                                            <button onClick={() => onConvertCaseType(redirectData.redirect)} className="mt-2 px-6 py-2.5 bg-red-600 dark:bg-red-700 text-white font-bold rounded-lg shadow-lg hover:bg-red-700 dark:hover:bg-red-800 transition-colors">نقل القضية فوراً</button>
                                        )}
                                    </div>
                                </div>
                            ) : (
                                ...
                                <div className="mt-4 pt-3 border-t border-gray-200 dark:border-slate-800 no-print">
                                    <div className="flex flex-wrap gap-2">
                                        {nextActions.map((action, idx) => (
                                            <button key={idx} onClick={() => onFollowUpAction(action.mode, action.prompt)} className="px-4 py-2 bg-gray-50 dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/40 border border-gray-300 dark:border-slate-700 rounded-lg text-sm text-gray-700 dark:text-slate-100 shadow-sm transition-all font-semibold">
                                                {action.label}
                                            </button>
                                        ))}
                                    </div>
                            ...
                            {hasGrounding && !redirectData && (
                                <div className="mt-6 pt-4 border-t border-gray-200 dark:border-slate-800 bg-gray-50 dark:bg-slate-950/40 rounded-lg p-3 -mx-2 no-print shadow-inner">
                                    <p className="text-xs font-bold text-blue-700 dark:text-blue-400 mb-3 flex items-center">
                                    ...
                                    <div className="space-y-2">
                                        {msg.groundingMetadata?.groundingChunks?.map((chunk, idx) => (
                                            chunk.web && (
                                                <a key={idx} href={chunk.web.uri} target="_blank" rel="noopener noreferrer" className="flex items-center p-2 rounded bg-white dark:bg-slate-800 hover:bg-blue-50 dark:hover:bg-blue-900/30 border border-gray-200 dark:border-slate-700 transition-all group/link">
                                                    <span className="flex-shrink-0 w-5 h-5 rounded bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-300 flex items-center justify-center text-[10px] me-2 font-mono border border-blue-200 dark:border-blue-700">{idx + 1}</span>
                                                    <div className="flex-grow min-w-0">
                                                        <p className="text-xs font-bold text-gray-800 dark:text-white truncate group-hover/link:text-blue-700 dark:group-hover/link:text-blue-400">{chunk.web.title || "مصدر قانوني"}</p>
                                                        <p className="text-[10px] text-gray-500 dark:text-slate-400 truncate font-mono">{new URL(chunk.web.uri).hostname}</p>
                                                    </div>
                                                    <svg xmlns="http://www.w3.org/2000/svg" className="h-3 w-3 text-gray-400 dark:text-slate-500 ms-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
                                                </a>
                                            )
                                        ))}
                                    </div>
