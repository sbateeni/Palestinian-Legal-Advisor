...
const CasesListPage: React.FC = () => {
    ...
    const handleCaseNavigation = (id: string) => {
        ...
        if (caseItem?.caseType === 'inheritance') {
            router.push(`/inheritance?caseId=${id}`);
        } else if (caseItem?.caseType === 'sharia') {
            router.push(`/sharia/${id}`);
        } else {
            router.push(`/case/${id}`);
        }
    };
        ...
        <div className="w-full p-4 dark:bg-slate-950 transition-colors duration-300">
            ...
            {displayedCases.length === 0 ? (
                <div className="text-center py-16">
                    ...
                    {(!searchTerm && statusFilter === 'all' && !startDate && !endDate) && (
                        <button onClick={() => router.push('/')} className="mt-4 px-6 py-2 bg-blue-600 dark:bg-blue-700 text-white font-semibold rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors">
                            ابدأ قضية جديدة
                        </button>
                    )}
                </div>
