...
import { generateTimelineFromChat } from './geminiService';
...
const ChatPage: React.FC<ChatPageProps> = ({ caseId }) => {
    ...
    // 1. Loading State
    if (isCaseHydrating && !caseData && caseId) {
        return (
    ...
    if (isNotFound) {
        return (
            <div className="w-full flex-grow flex flex-col items-center justify-center text-center p-4">
                ...
                <button onClick={() => router.push('/cases')} className="px-6 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                    العودة للقضايا
                </button>
            </div>
        );
            ...
            <div className="w-full flex-grow flex flex-col items-center justify-center text-center p-4">
                ...
                {isGemini ? (
                    <p className="text-gray-600 mb-6 max-w-2xl">
                        لاستخدام Gemini، يرجى تحديد مفتاح Google AI API الخاص بك عبر النافذة المنبثقة، أو إدخاله يدويًا في <Link href="/settings" className="text-blue-600 hover:underline">صفحة الإعدادات</Link>.
                    </p>
                ) : (
                    <p className="text-gray-600 mb-6 max-w-2xl">
