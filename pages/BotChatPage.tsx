              ...
              <div className={`max-w-xl lg:max-w-3xl px-5 py-3 rounded-2xl relative ${msg.role === 'user' ? 'bg-blue-600 text-white rounded-br-none' : 'bg-gray-700 text-gray-200 rounded-bl-none'}`}>
                ...
                {msg.role === 'model' ? (
                   <>
                    ...
                    {/* FIX: Use `marked.parse()` as `marked.parseSync()` is deprecated. Cast to string to handle potential async return type. */}
                    <div className="prose prose-invert max-w-none" dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(marked.parse(msg.content || '...') as string) }}></div>
                   </>
                 ) : (
